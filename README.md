...
**jbyrdrh Windows note**: In the Ansible Automation Controller UI, it is important to note that you canot set the winrm connection details for the Windows client at the job template level. Instead, I would recommend setting it at the **host group level** that you create for your Windows managed nodes. If you set the credentails at the job template, then both hosts will try to use those credentials (both the Windows client *and* the localhost).
